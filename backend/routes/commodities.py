...
def get_commodities() -> List[Commodity]:
    live = live_commodities()
    return live if live else mock_commodities()
...
def commodity_detail_route(symbol: str):
    ...
    return CommodityDetail(
        **match.model_dump(),
        ...
        history_30d=match.sparkline if len(match.sparkline) >= 20 else sparkline(match.price, points=30, vol=0.02),
