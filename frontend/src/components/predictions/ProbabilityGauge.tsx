  ...
  return (
    <div className="flex flex-col items-center" data-testid="probability-gauge">
      <svg viewBox="0 0 200 120" width={size} height={size * 0.7}>
        ...
        <path d="M 20 100 A 80 80 0 0 1 180 100" fill="none" stroke="url(#probArc)" strokeWidth="10" strokeLinecap="round" />
        <motion.line
      ...
      <div className="mt-1 text-center">
        <div className="font-serif text-3xl leading-none" style={{ color: isBull ? bullColor : bearColor }}>{bullish}%</div>
        <div className="font-mono text-[10px] uppercase tracking-widest text-white/50 mt-1">Bullish · {bearish}% Bearish</div>
        <div className="font-mono text-[10px] uppercase tracking-[0.2em] mt-2 px-2 py-0.5 inline-block rounded border" style={{
          color: isBull ? bullColor : bearColor,
          borderColor: isBull ? 'rgba(16,185,129,0.4)' : 'rgba(239,68,68,0.4)',
          background: isBull ? 'rgba(16,185,129,0.06)' : 'rgba(239,68,68,0.06)',
        }}>
          {label}
        </div>
      </div>
