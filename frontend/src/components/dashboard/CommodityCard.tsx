...
export function CommodityCard({ c, index = 0 }: { c: Commodity; index?: number }) {
  ...
  return (
    ...
    >
      <div className="absolute -top-6 -right-6 h-24 w-24 rounded-full opacity-[0.06] group-hover:opacity-[0.12] transition"
        style={{ background: `radial-gradient(circle, ${stroke}, transparent 70%)` }} />
      ...
      <div className="mt-4 flex items-baseline gap-3">
        <div className="font-serif text-3xl md:text-4xl tracking-tight" data-testid={`commodity-price-${c.symbol.toLowerCase()}`}>
          ${fmt(c.price)}
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-white/[0.06] flex items-center justify-between">
        <div>
          <div className="font-mono text-[9px] uppercase tracking-widest text-white/40">7d Trend</div>
          <div className={`text-sm font-medium ${c.change_7d >= 0 ? 'text-bull' : 'text-bear'}`}>{fmtPct(c.change_7d)}</div>
        </div>
        <div className="text-right">
          <div className="font-mono text-[9px] uppercase tracking-widest text-white/40">AI Impact</div>
            ...
            <div className="h-1 w-16 bg-white/[0.06] rounded-full overflow-hidden">
              <div
                ...
                style={{
                  ...
                  background: c.ai_impact_score >= 70 ? '#D4AF37' : c.ai_impact_score >= 40 ? '#F59E0B' : '#3B82F6',
                }}
              />
            </div>
            <span className="font-mono text-xs text-white/80">{c.ai_impact_score}</span>
          </div>
        </div>
      </div>
    </motion.div>
