...
export function RiskMeter({ value }: { value: number }) {
  ...
  const levelColor = value >= 70 ? '#EF4444' : value >= 40 ? '#F59E0B' : '#10B981';

  return (
    <div className="glass rounded-2xl p-6" data-testid="risk-meter">
      <div className="flex items-baseline justify-between">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-editorial-gold/80">Global</div>
      ...
      <div className="relative h-40 flex items-end justify-center">
        ...
        <motion.div
          ...
          style={{ background: levelColor, boxShadow: `0 0 10px ${levelColor}` }}
        />
        <div className="absolute left-1/2 bottom-[10px] -translate-x-1/2 h-3 w-3 rounded-full bg-white/80" />
      </div>
      <div className="text-center -mt-2">
        <div className="font-serif text-4xl" data-testid="risk-value">{value}</div>
        <div className="font-mono text-[10px] uppercase tracking-widest mt-1" style={{ color: levelColor }}>
          {level}
        </div>
      </div>
    </div>
