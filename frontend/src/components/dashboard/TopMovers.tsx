...
type Mover = { symbol: string; name: string; change: number; direction: 'up' | 'down' };
  ...
  return (
    <div className="glass rounded-2xl p-6" data-testid="top-movers">
      <div className="flex items-baseline justify-between">
        ...
        <span className="font-mono text-[10px] uppercase tracking-widest text-white/40">24H</span>
      </div>
      ...
      <ul className="space-y-2">
        {movers.map((m, i) => {
          ...
          return (
            ...
            >
              <div className={`h-7 w-7 rounded-md flex items-center justify-center ${up ? 'bg-bull/10 text-bull' : 'bg-bear/10 text-bear'}`}>
                {up ? <ArrowUp className="h-3.5 w-3.5" /> : <ArrowDown className="h-3.5 w-3.5" />}
              </div>
              <div className="min-w-0 flex-1">
                <div className="font-mono text-[10px] uppercase tracking-widest text-white/40">{m.symbol}</div>
                <div className="text-sm text-white/90 truncate">{m.name}</div>
              </div>
              <div className={`font-mono text-sm ${up ? 'text-bull' : 'text-bear'}`}>{fmtPct(m.change)}</div>
            </motion.li>
          );
        })}
      </ul>
