...
export function CountryDetail({ country, onClose }: { country: CountryRisk; onClose: () => void }) {
  return (
    <motion.aside
      key={country.code}
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.22em] text-editorial-gold/80">Country · {country.code}</div>
          <h2 className="font-serif text-3xl mt-1 tracking-tight">{country.name}</h2>
        </div>
        <button
      ...
      <div className="font-mono text-[10px] uppercase tracking-widest text-white/50 mb-3">Recent Events</div>
      ...
      <div className="font-mono text-[10px] uppercase tracking-widest text-white/50 mb-3">Affected Commodities</div>
      <div className="flex flex-wrap gap-2" data-testid="country-commodities">
        {country.affected_commodities.map((c) => (
          <span key={c} className="text-xs font-mono uppercase text-editorial-gold/90 border border-editorial-gold/30 rounded-md px-2 py-1 bg-editorial-gold/5">
            {c}
          </span>
        ))}
      </div>
