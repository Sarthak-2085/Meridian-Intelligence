...
import { LayoutDashboard, Newspaper, LineChart, Globe2, Sparkles, Settings, Compass } from 'lucide-react';
...
const NAV = [
  ...
  { href: '/commodities', label: 'Commodities', icon: LineChart, testid: 'nav-commodities' },
  { href: '/countries', label: 'Countries', icon: Globe2, testid: 'nav-countries' },
  { href: '/predictions', label: 'Predictions', icon: Sparkles, testid: 'nav-predictions' },
    ...
    >
      <div className="px-6 pt-8 pb-6 flex items-center gap-3">
        ...
        <div className="flex flex-col leading-none">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.22em] text-white/40 mt-1">Intelligence</span>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-6 space-y-0.5">
        {NAV.map((n) => {
          ...
          return (
            ...
            >
              <Icon className={cn('h-4 w-4', active ? 'text-editorial-gold' : 'text-white/40 group-hover:text-white/70')} />
              <span className="font-medium">{n.label}</span>
              {active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-editorial-gold" />}
            </Link>
          );
        })}
      </nav>
      <div className="mx-4 mb-6 p-4 rounded-xl glass">
        <div className="font-mono text-[10px] uppercase tracking-widest text-white/40 mb-1">System</div>
        ...
        <div className="mt-3 text-[11px] text-white/40">Mock data · No paid APIs</div>
      </div>
    </aside>
  );
}
