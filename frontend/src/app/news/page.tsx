...
const IMPACTS: Array<'all' | 'high' | 'medium' | 'low'> = ['all', 'high', 'medium', 'low'];
...
export default function NewsPage() {
  ...
  const regions = useMemo(() => ['All', ...Array.from(new Set(news.map((n) => n.region)))], [news]);
  ...
  const filtered = useMemo(() => {
    ...
    return news.filter((n) => {
      ...
      if (needle && !(n.headline.toLowerCase().includes(needle) || n.source.toLowerCase().includes(needle))) return false;
      return true;
    });
    ...
    <div className="space-y-6" data-testid="news-page">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <h1 className="font-serif text-4xl md:text-5xl mt-2 tracking-tight">
          Every headline, <em className="text-editorial-gold not-italic">contextualised.</em>
        </h1>
        <p className="text-white/55 mt-3 max-w-2xl text-[15px]">Filter by impact, region, and free-text search.</p>
      </motion.div>
          ...
          {IMPACTS.map((i) => (
            <button
              ...
              className={`h-10 px-3 rounded-lg font-mono text-[10px] uppercase tracking-widest transition border ${
                impact === i ? 'bg-editorial-gold text-ink-950 border-editorial-gold' : 'text-white/60 border-white/[0.08] hover:bg-white/[0.05]'
              }`}
            >
              {i}
            </button>
      ...
      {/* List */}
      <div className="glass rounded-2xl divide-y divide-white/[0.05]" data-testid="news-list">
        {loading && Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-24 m-3" />)}
        {!loading && filtered.length === 0 && (
          <div className="p-10 text-center text-white/50 text-sm">No headlines match those filters.</div>
        )}
        {!loading && filtered.map((n, i) => (
          ...
          >
            <div className="h-20 w-28 shrink-0 rounded-lg bg-cover bg-center border border-white/5" style={{ backgroundImage: `url(${n.thumbnail})` }} />
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1.5">
                <span className={`text-[10px] font-mono uppercase tracking-widest px-1.5 py-0.5 rounded border ${impactStyles[n.impact]}`}>{n.impact}</span>
                <span className="font-mono text-[10px] uppercase tracking-widest text-white/40">{n.source} · {n.region} · {n.category}</span>
                <span className="font-mono text-[10px] uppercase tracking-widest text-white/30 ml-auto">{timeAgo(n.timestamp)}</span>
              </div>
              <div className="font-serif text-lg leading-snug text-white/95 group-hover:text-white">{n.headline}</div>
              <div className="mt-2 flex flex-wrap gap-1">
                {n.affected_commodities.map((c) => (
                  <span key={c} className="text-[10px] font-mono uppercase text-editorial-gold/80 border border-editorial-gold/20 rounded px-1.5">{c}</span>
                ))}
              </div>
            </div>
