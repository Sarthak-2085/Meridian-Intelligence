...
const InteractiveMap = dynamic(
  () => import("@/components/countries/InteractiveMap"),
  {
    ssr: false,
...
export default function CountriesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return countries.filter(
      (c) =>
        c.name.toLowerCase().includes(n) || c.code.toLowerCase().includes(n),
    );
  }, [q, countries]);
    ...
    <div className="space-y-6" data-testid="countries-page">
      ...
      >
        ...
        <h1 className="font-serif text-4xl md:text-5xl mt-2 tracking-tight">
          Every country is a{" "}
          <em className="text-editorial-gold not-italic">signal</em>.
        </h1>
        <p className="text-white/55 mt-3 max-w-2xl text-[15px] leading-relaxed">
          Click a marker to open its political &amp; economic risk breakdown,
          recent events, and commodities exposed to it.
        </p>
      </motion.div>

      {err && (
        <div
          className="glass rounded-2xl p-6 text-bear text-sm"
          data-testid="countries-error"
        >
          {err}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div
            className="glass rounded-2xl p-2 max-h-64 overflow-y-auto"
          ...
          >
            {filtered.map((c) => (
              ...
              >
                <span className="font-mono text-[10px] uppercase tracking-widest text-white/40 w-8">
                  {c.code}
                </span>
