...
import { AlertCircle, ArrowLeft, TrendingDown, TrendingUp, MapPin } from 'lucide-react';
...
export default function CommodityDetailPage() {
  ...
  const [timeframe, setTimeframe] = useState<(typeof TIMEFRAMES)[number]['key']>('history_30d');
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="glass rounded-2xl p-6">
        ...
        <div className="mt-5 flex items-baseline gap-3">
          <div className="font-serif text-4xl md:text-5xl tracking-tight">${fmt(data.price)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        >
          ...
          {data.exposed_countries.length === 0 ? (
            <p className="text-white/40 text-sm">No significant country-level exposure detected right now.</p>
          ) : (
        ...
        >
          ...
          {data.related_news.length === 0 ? (
            <p className="text-white/40 text-sm">No related headlines right now.</p>
          ) : (
            <div className="space-y-3">
              {data.related_news.map((n) => (
                <div key={n.id} className="pb-3 border-b border-white/[0.06] last:border-0 last:pb-0">
                  <div className="text-sm text-white/85 leading-snug">{n.headline}</div>
