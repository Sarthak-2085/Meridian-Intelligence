...
export default function SettingsPage() {
  ...
  const toggleWatch = (code: string) => {
    ...
    setSettings({
      ...settings,
      watchlist: has
        ? settings.watchlist.filter((x) => x !== code)
        : [...settings.watchlist, code],
    });
  };
    ...
    <div className="space-y-6 max-w-3xl" data-testid="settings-page">
      ...
      >
        ...
        <p className="text-white/55 mt-3 max-w-2xl text-[15px]">
          Choose what to watch, when to be alerted, and where to focus. Saved to
          your workstation.
        </p>
      </motion.div>
            ...
            return (
              <button
                ...
                className={`text-left rounded-lg px-4 py-3 border transition ${
                  active
                    ? "bg-editorial-gold/10 border-editorial-gold/40"
                    : "bg-white/[0.02] border-white/[0.08] hover:bg-white/[0.05]"
                }`}
              >
                <div className="flex items-center gap-2">
                  <span
                    className={`h-4 w-4 rounded flex items-center justify-center border ${active ? "bg-editorial-gold border-editorial-gold" : "border-white/25"}`}
                  >
      ...
      <div className="glass rounded-2xl p-6" data-testid="settings-threshold">
        ...
        <div className="flex items-baseline justify-between">
          <span className="text-sm text-white/60">
            Notify me when overall country risk exceeds
          </span>
          <span
            className="font-serif text-3xl text-editorial-gold"
        ...
        <input
          ...
          onChange={(e) =>
            setSettings({ ...settings, risk_threshold: Number(e.target.value) })
          }
